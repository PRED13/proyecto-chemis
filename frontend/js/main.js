/**
 * Lógica Global de Granja Premium
 * Maneja: Autenticación, Catálogo, Carrito y UI Dinámica.
 */
        ...
        try {
            ...
            authContainer.innerHTML = `
                <div class="flex items-center gap-4">
                    <a href="dashboard.html" class="text-sm font-medium hover:text-[#D65D46] transition">
                        Hola, <span class="font-bold">${payload.nombre}</span>
                    </a>
                    <button onclick="logout()" class="text-sm text-red-500 hover:underline cursor-pointer">
                        Cerrar Sesión
                    </button>
                </div>
            `;
            ...
            <div class="p-6 flex-grow flex flex-col">
                ...
                <div class="mt-auto">
                    ...
                    <div class="flex gap-2">
                        <button onclick="verDetalles(${ave.id})" class="flex-1 border border-gray-200 py-2 rounded-lg text-sm font-medium hover:bg-gray-50 transition cursor-pointer">
                            Detalles
                        </button>
                        <button onclick='agregarAlCarrito(${aveString})' 
                                class="flex-[1.5] bg-[#D65D46] text-white py-2 rounded-lg text-sm font-medium hover:bg-[#b84d39] transition cursor-pointer">
                            + Añadir
                        </button>
                    </div>
                </div>
...
function agregarAlCarrito(producto) {
    ...
    if (index !== -1) {
        if (carrito[index].cantidad < producto.stock) {
            ...
            if (typeof toastr !== 'undefined') toastr.success(`${producto.nombre} actualizado`);
        } else {
            if (typeof toastr !== 'undefined') toastr.error("Límite de stock alcanzado");
            return;
        }
    } else {
